    ...
    for attempt in range(max_retries):
        try:
            ...
            is_rate_limit = (
                ...
                'rate-limited' in error_str
            )
            
            if is_rate_limit and attempt < max_retries - 1:
                wait_time = delay * (attempt + 1)  # Incremental backoff
                print(f"[RATE LIMIT] Attempt {attempt + 1}/{max_retries} failed. Retrying in {wait_time}s...")
...
def is_job_application(subject: str, sender: str, text: str) -> bool:
    ...
    notification_keywords = [
        'job alert', 'new jobs', 'job recommendation', 'might be interested',
        'top matches', 'jobs match', 'hiring for', 'has new', 'jobs open',
        'be the first to apply', 'just posted', '% match from jobright',
        'jobs like you', 'job opportunities', 'recommended for you'
    ]
    
    other_keywords = [
        'github actions', 'workflow', 'run failed', 'federating', 'sso',
        'identity provider', 'newsletter', 'community update'
    ]
    ...
    if any(keyword in subject_lower or keyword in text_lower for keyword in other_keywords):
        return False
    ...
    application_patterns = [
        'application received', 'application submitted', 'application status',
        'interview', 'offer', 'unfortunately', 'not moving forward',
        'thank you for applying', 'your application to', 'application for',
        'progress with other candidates', 'pursue other candidates',
        'not selected', 'position has been filled', 'we regret'
    ]
    
    if any(pattern in subject_lower or pattern in text_lower for pattern in application_patterns):
        return True
    
    # Now filter out obvious job alert notifications
    # (only if no application keywords were found above)
    if any(keyword in subject_lower or keyword in text_lower for keyword in notification_keywords):
        return False
