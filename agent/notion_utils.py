...
def find_entry_by_company_title(company: str, job_title: str):
    ...
    try:
        ...
        for page in resp.get("results", []):
            ...
            if page_title and page_title[0].get("text", {}).get("content", "") == job_title:
                return page
...
def create_or_update_entry(company: str, job_title: str, status: str, applied_on: str, 
                           notes: str = "", app_id: str = None):
    """
    ...
    if not company or not job_title:
        print(f"[ERROR] Missing required fields: company='{company}', job_title='{job_title}'")
        return None, False
    ...
    if existing_page:
        try:
            old_status = existing_page.get("properties", {}).get("Status", {}).get("status", {}).get("name", "Unknown")
            ...
            if app_id:
                update_props["Application ID"] = {"rich_text": [{"text": {"content": app_id}}]}
            
            # Append to notes if new information
            if notes:
                old_notes = existing_page.get("properties", {}).get("Notes", {}).get("rich_text", [])
                old_notes_text = old_notes[0].get("text", {}).get("content", "") if old_notes else ""
    ...
    try:
        ...
        if app_id:
            properties["Application ID"] = {"rich_text": [{"text": {"content": app_id}}]}
        ...
        print(f"   Check: 1) Database exists, 2) Integration has access, 3) Properties match")
        import traceback
        traceback.print_exc()
        return None, False

# === LEGACY: CREATE ENTRY (for backward compatibility) ===
def create_entry(company: str, job_title: str, status: str, applied_on: str, notes: str = ""):
    """
    Legacy function for backward compatibility.
    Now calls create_or_update_entry without app_id.
    """
    result, _ = create_or_update_entry(company, job_title, status, applied_on, notes, app_id=None)
    return result

# === QUERY ENTRIES (used for weekly summary later) ===
...
def get_weekly_application_data(days: int = 7):
    ...
    try:
        ...
        for entry in entries:
            ...
            company_name = company[0].get("text", {}).get("content", "Unknown") if company else "Unknown"
            
            job_title = props.get("Job Title", {}).get("rich_text", [])
            job_title_text = job_title[0].get("text", {}).get("content", "Unknown") if job_title else "Unknown"
            ...
            deadline_keywords = ["deadline", "due", "by", "before", "respond by", "complete by"]
